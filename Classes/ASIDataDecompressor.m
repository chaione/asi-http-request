...
+ (BOOL)uncompressDataFromFile:(NSString *)sourcePath toFile:(NSString *)destinationPath error:(NSError **)err
{
    ...
    while ([decompressor streamReady]) {
		...
		if ([inputStream streamStatus] == NSStreamEventErrorOccurred) {
			if (err) {
                NSString *msg =[NSString stringWithFormat:@"Decompression of %@ failed because we were unable to write to the destination data file at %@",sourcePath,destinationPath];
				*err = [NSError errorWithDomain:NetworkRequestErrorDomain code:ASICompressionError userInfo:
